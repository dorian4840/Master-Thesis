...
class IMPALA_dataset(Dataset):
    def __init__(self, X_clinical, X_vital, y_avpu, y_crt):
        self.X_clinical = X_clinical
        self.X_vital = X_vital
        self.y_avpu = y_avpu
        self.y_crt = y_crt

    def __len__(self):
        return self.X_clinical.shape[0]

    def __getitem__(self, idx):
        return self.X_clinical[idx, :, :], self.X_vital[idx, :, :], \
                self.y_avpu[idx, :], self.y_crt[idx, :]
...
def create_datasets(dataset, data_split):
    ...
    clinical_X = np.concatenate([v['clinical_X'] for k, v in dataset.items() if k in train_ids], axis=0)
    vital_X = np.concatenate([v['vital_X'] for k, v in dataset.items() if k in train_ids], axis=0)
    ...
    train_set = IMPALA_dataset(torch.from_numpy(clinical_X), torch.from_numpy(vital_X),
                               torch.from_numpy(y_avpu), torch.from_numpy(y_crt))

    # validation
    val_size = int(len(random_ids) * data_split[1])
    val_ids = random_ids[train_size:train_size+val_size]
    clinical_X = np.concatenate([v['clinical_X'] for k, v in dataset.items() if k in val_ids], axis=0)
    vital_X = np.concatenate([v['vital_X'] for k, v in dataset.items() if k in val_ids], axis=0)
    ...
    val_set = IMPALA_dataset(torch.from_numpy(clinical_X), torch.from_numpy(vital_X),
                               torch.from_numpy(y_avpu), torch.from_numpy(y_crt))

    # test
    test_ids = random_ids[train_size+val_size:]
    clinical_X = np.concatenate([v['clinical_X'] for k, v in dataset.items() if k in test_ids], axis=0)
    vital_X = np.concatenate([v['vital_X'] for k, v in dataset.items() if k in test_ids], axis=0)
    ...
    test_set = IMPALA_dataset(torch.from_numpy(clinical_X), torch.from_numpy(vital_X),
                               torch.from_numpy(y_avpu), torch.from_numpy(y_crt))
